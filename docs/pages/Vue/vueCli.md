...
在`../lib/create`中，会先对`项目名称`、`项目目标位置`进行检查，然后执行：
...
在`@vue/cli/lib/Creator.js`构造函数中：
...
由此可知，上面 `@vue/cli/lib/promptModules/babel.js` 中的参数`cli`其实是 `PromptModuleAPI` 的实例。`injectFeature`的作用是将功能特性注册到 `Creator` 实例中，而 `onPromptComplete` 则是注册了回调函数。

接下来继续看向`lib/Creator.js`中`Create`这个类的逻辑
